...
const EditorInner = (props: Props) => {
  ...
  const ss = useSyncedStore(store);

  console.log(JSON.stringify(ss));

  // UGH todo
  if (!ss.promptDocuments || ss.promptDocuments.editor1 === undefined) {
        ...
        <div className="editor-main-column-header">
          <div className="editor-main-column-header-title">
            ...
            <Tooltip
              title={
                <T>
                  You can use the system prompt to give general instructions to
                  the model independent of the specific chat prompt. For
            ...
            <Tooltip
              title={
                <T>
                  ...
                  <p>
                    Typically, the last item in a prompt is a &ldquo;User&rdquo;
                    section, representing the current query. The model will
                    always respond with an &ldquo;Assistant&rdquo; section.
                  </p>
                  <p>
                    Even if you are not building a chatbot, you can use the chat
                    format to delineate &rdquo;turns&ldquo; taken by the system.
                    &ldquo;User&rdquo; sections do not need to be formatted as
                    natural-language questions.
                  </p>
                  <p>
                    Many of the highest-quality large language models are
                    available only in chat format, so it can be valuable to use
                    chat prompts even if your prompt only ever consists of a
                    single &ldquo;User&rdquo; turn.
                  </p>
                </T>
              }
            >
...
const Editor: React.FC<Props> = (props) => {
  ...
  // const hasPopulatedStore = useHasPopulatedStore(storeAndWebsocketProvider);
  ...
  return (
    <StoreContext.Provider
      value={storeAndWebsocketProvider}
    >
      <EditorInner {...props} />
    </StoreContext.Provider>
  );
